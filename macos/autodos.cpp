// autodos.cpp — AutoDOS core library implementation (Cross-platform)
//
// AutoDOS - Original by makuka97 (https://github.com/makuka97)
// macOS port by cargabsj175 (vibe coding approach)
//
// This is a cross-platform adaptation of the original Windows version.
// All Windows-specific APIs (FindFirstFile, CreateProcess, etc.) have been
// replaced with POSIX/macOS equivalents (opendir/readdir, fork/exec, etc.).
//
// The original src/autodos.cpp remains completely untouched.
// This file lives in macos/ and is built only for the macOS port.
...
bool extractZip(const std::string& zipPath, const std::string& outDir) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        std::string dir = getParentDir(outPath);
        createDirs(dir);
...
bool launchDosBox(const std::string& dosboxPath, const std::string& confPath) {
    // Fork and exec DOSBox
    pid_t pid = fork();
    if (pid == 0) {
        // Child process - launch DOSBox
